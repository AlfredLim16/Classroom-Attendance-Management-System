...
public enum MissedQuizStatus {
    PENDING(1, "Pending"), APPROVED(2, "Approved"), REJECTED(3, "Rejected"), EXCUSED(4, "Excused");
    ...
    public static MissedQuizStatus fromId(int id) throws ValidationException {
        for (MissedQuizStatus m : values()) {
            if (m.missedQuizStatusId == id) {
                return m;
            }
        }
        throw new ValidationException("missedQuizStatusId", "Unknown missed quiz status id: " + id);
    }
}
