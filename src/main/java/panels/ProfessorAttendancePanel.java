    ...
    private void exportAttendanceToCSV(){
        if(attendanceTable.getRowCount() == 0){
            JOptionPane.showMessageDialog(this, 
                "No attendance data to export. Please load a session first.", 
                "No Data", 
                JOptionPane.INFORMATION_MESSAGE);
            return;
        }

        // Build default filename based on selected section and session
        String defaultFileName = "attendance_export.csv";
        if(cmbSection.getSelectedIndex() > 0 && cmbSession.getSelectedIndex() > 0){
            String sectionCode = cmbSection.getSelectedItem().toString().replaceAll("[^a-zA-Z0-9_-]", "_");
            String sessionInfo = cmbSession.getSelectedItem().toString()
                .replaceAll("[^a-zA-Z0-9_-]", "_")
                .replace("__", "_");
            defaultFileName = "attendance_" + sectionCode + "_" + sessionInfo + ".csv";
        }
        ...
        try{
            ...
            for(int i = 0; i < attendanceTable.getRowCount(); i++){
                for(int j = 0; j < attendanceTable.getColumnCount(); j++){
                    ...
                    if(j < attendanceTable.getColumnCount() - 1){
                        csv.append(",");
                    }
                }
                csv.append("\n");
            ...
            // Write to file
            java.nio.file.Files.writeString(fileToSave.toPath(), csv.toString());
