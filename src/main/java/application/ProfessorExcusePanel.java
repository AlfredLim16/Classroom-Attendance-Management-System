...
import excuse.ExcuseLetterService;
...
public class ProfessorExcusePanel extends JPanel implements ActionListener {
    ...
    private void loadExcuses(){
        ...
        List<ExcuseLetter> all = excuseService.getAllExcuseLetters();
    ...
    private void reviewSelected(ExcuseStatus newStatus){
        ...
        String currentStatus = letter.status().toString();
        ...
        if(confirm == JOptionPane.YES_OPTION){
            boolean success = excuseService.reviewExcuseLetter(
                letter.excuseId(), currentUser, newStatus, LocalDateTime.now());
            if(success){
                JOptionPane.showMessageDialog(this, "Excuse letter " + action + "d successfully.", "Success", JOptionPane.INFORMATION_MESSAGE);
                loadExcuses();
            }else{
                JOptionPane.showMessageDialog(this, "Failed to update excuse letter.", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
