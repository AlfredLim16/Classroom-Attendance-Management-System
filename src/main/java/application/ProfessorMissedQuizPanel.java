...
import assessment.DecisionType;
import assessment.MissedQuizFlag;
import assessment.MissedQuizFlagService;
import assessment.MissedQuizProcessingService;
...
public class ProfessorMissedQuizPanel extends JPanel implements ActionListener {
    ...
    private final MissedQuizProcessingService processingService = new MissedQuizProcessingService();
    ...
    private void showDetailDialog(MissedQuizFlag flag){
        ...
        y += gap;
        addDetailRow(dialog, "Status", flag.status().toString(), y);
        y += gap;
        addDetailRow(dialog, "Decision", flag.decisionType() != null ? flag.decisionType().toString() : "Pending", y);
        y += gap;
        addDetailRow(dialog, "Decided By", flag.decidedBy() != null ? flag.decidedBy().getFullName() : "-", y);
        y += gap;
        ...
        btnClose.setBounds(370, 340, 120, 36);
    ...
    private void submitDecision(){
        ...
        DecisionType decisionType = switch(decisionIndex){
            ...
            case 2 ->
                DecisionType.FAILED;
            case 3 ->
                DecisionType.EXCUSED_ABSENCE;
            case 4 ->
                DecisionType.EXEMPTED;
            default ->
                null;
        };
        ...
        if(confirm == JOptionPane.YES_OPTION){
            boolean success = processingService.processMissedQuizFlag(
