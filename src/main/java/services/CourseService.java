...
public class CourseService {
    ...
    public Course createCourse(Program program, String code, String name, byte units, Semester semester, YearLevel yearLevel){
        try{
            Course course = Course.builder().program(program).courseCode(code).courseName(name).units(units).semester(semester).yearLevel(yearLevel).build();
            courseDAO.insert(course);
            return courseDAO.findAll().stream().filter(c -> c.courseCode().equals(code)).findFirst().orElse(null);
